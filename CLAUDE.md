...
Parallel to the three layers above, the Sanity schema itself is exported as a Mermaid `classDiagram` to [docs/content-model.md](docs/content-model.md). Generator lives in its own top-level workspace at [content-model/](content-model/) — TypeScript, Vitest-tested, structured so the same modules can later move to a Studio plugin without rewriting the core. Run it with `pnpm --filter uxmethods-content-model generate`. Unlike the `graph/build/*` artefacts, the generated file is **committed** — its git history is the drift-tracking signal. It is not pushed to Fuseki, not imported into `workspace.ttl`, and not involved in inference. See [ADR 0006](docs/decisions/0006-content-model-mermaid-export.md) for mapping rules, skip patterns, and the rationale for picking Mermaid over the OWL/RDFS approach that ADR 0005 (on `archive/content-model-rdf`) attempted.

**Known limitation of the CLI path:** types contributed by Studio plugins (e.g. `skosConcept` from `sanity-plugin-taxonomy-manager`) aren't visible to the CLI because it loads `studio/schemaTypes/index.ts` directly, before the workspace's plugin schema is composed. Fields referencing those types appear in their parent classes (the type label is still shown), but their edges drop with warnings. The future Studio plugin form of this tool will use `useSchema()` to access the fully-composed schema and the limitation goes away. See ADR 0006's "Generator" section.
