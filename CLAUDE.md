...
Parallel to the three layers above, the Sanity schema can be visualised as a Mermaid `classDiagram` — documents vs objects, fields with cardinality, composition and reference edges. This began as an in-repo CLI (`content-model/`) that generated a committed `docs/content-model.md`, but the CLI had a hard limitation: loading `studio/schemaTypes/index.ts` directly, it never saw types contributed by Studio plugins (e.g. `skosConcept` from `sanity-plugin-taxonomy-manager`), so those edges dropped. **The CLI and its generated artefact have been retired** in favour of the Studio plugin below, which reads the fully-composed schema and has no such blind spot. The Mermaid mapping contract the CLI established lives on — see [ADR 0006](docs/decisions/0006-content-model-mermaid-export.md) (amended to note the retirement; it also covers the rationale for Mermaid over the OWL/RDFS approach ADR 0005 attempted).

**The content model now lives in a published Studio plugin:** [`sanity-plugin-mermaid-content-model`](https://github.com/andybywire/sanity-plugin-mermaid-content-model) — extracted from this monorepo ([ADR 0007](docs/decisions/0007-content-model-plugin-architecture.md)) and published to npm. It renders the diagram **in-Studio** (the **Content Model** top-nav tool), reading the composed workspace schema via `useSchema()`, so `skosConcept` and other plugin-contributed types are included. `studio/` consumes it (`^0.1.0`). Like the old artefact, it sits parallel to the three KG layers — not pushed to Fuseki, not imported into `workspace.ttl`, not involved in inference.
